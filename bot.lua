...
function get_bot (i, dark)
	function bot_info (i, dark)
		redis:set("botBOT-IDid",dark.id_)
		if dark.first_name_ then
			redis:set("botBOT-IDfname",dark.first_name_)
		end
		if dark.last_name_ then
			redis:set("botBOT-IDlanme",dark.last_name_)
		end
		redis:set("botBOT-IDnum",dark.phone_number_)
		return dark.id_
	end
...
function is_dark(msg)
    local var = false
	local hash = 'botBOT-IDadmin'
	local user = msg.sender_user_id_
    local dark = redis:sismember(hash, user)
	if dark then
		var = true
	end
	return var
...
function process_join(i, dark)
	if dark.code_ == 429 then
		local message = tostring(dark.message_)
...
function process_link(i, dark)
	if (dark.is_group_ or dark.is_supergroup_channel_) then
		redis:srem("botBOT-IDwaitelinks", i.link)
		redis:sadd("botBOT-IDgoodlinks", i.link)
	elseif dark.code_ == 429 then
		local message = tostring(dark.message_)
		local Time = message:match('%d+')
			...
			if not redis:sismember("botBOT-IDalllinks", link) then
				...
				elseif (text:match("^(/fwd) (.*)$") and msg.reply_to_message_id_ ~= 0) then
					...
					if matches:match("^(all)$") then
						dark = "botBOT-IDall"
					elseif matches:match("^(user)") then
						dark = "botBOT-IDusers"
					elseif matches:match("^(group)$") then
						dark = "botBOT-IDgroups"
					elseif matches:match("^(sgps)$") then
						dark = "botBOT-IDsupergroups"
					else
						return true
					end
					local list = redis:smembers(dark)
					...
					return send(msg.chat_id_, msg.id_, "<i>send</i>")
				elseif text:match("^(/fwd sgps) (.*)") then
					local matches = text:match("^/fwd sgps (.*)")
			...
			if is_dark(msg) then
				...
				elseif text:match("^(لیست) (.*)$") then
					...
					if matches == "مخاطبین" then
						...
						function (I, dark)
							local count = dark.total_count_
							local text = "مخاطبین : \n"
							for i =0 , tonumber(count) - 1 do
								local user = dark.users_[i]
					...
					writefile(tostring(dark)..".txt", text)
					tdcli_function ({
						ID = "SendMessage",
						...
						input_message_content_ = {ID = "InputMessageDocument",
							document_ = {ID = "InputFileLocal",
							path_ = tostring(dark)..".txt"},
						caption_ = "/link"..tostring(matches).." های تبلیغ گر شماره BOT-ID"}
					}, dl_cb, nil)
					return io.popen("rm -rf "..tostring(dark)..".txt"):read("*all")
				...
				elseif text:match("^(تازه سازی)$")then
					...
					tdcli_function({
						...
						redis:set("botBOT-IDcontacts", dark.total_count_)
					end, nil)
					for i, v in pairs(list) do
							for a, b in pairs(v) do 
								tdcli_function ({
									...
									if  dark.ID == "Error" then rem(i.id) 
									end
								end, {id=b})
							end
					end
				...
				elseif tostring(msg.chat_id_):match("^-") then
					...
					elseif text:match("^(افزودن همه مخاطبین)$") then
						tdcli_function({
							...
							local users, count = redis:smembers("botBOT-IDusers"), dark.total_count_
							for n=0, tonumber(count) - 1 do
								tdcli_function ({
									ID = "AddChatMember",
									chat_id_ = i.chat_id,
									user_id_ = dark.users_[n].id_,
									forward_limit_ = 50
								},  dl_cb, nil)
							end
							for n=1, #users do
